...
public class GuiClickGUI extends GuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		client.Client.themeManager.setTheme(((ClickGUI) ModuleManager.getModulebyClass(ClickGUI.class)).theme.getMode());
			...
			case 1:
				...
				for (int x = 0; x<((int)((width-100)/size)); x++) {
					for (int y = 0; y<(int)((height-50)/size); y++) {
						if (mc.theWorld.getHeight()>mc.thePlayer.posY);
						else continue;
				...
				for (Entity ent : mc.theWorld.loadedEntityList) {
					...
					int dx = (int) (ent.posX - mc.thePlayer.posX)+((int)((width-100)/size))/2
							,dz = (int) (ent.posZ - mc.thePlayer.posZ)+((int)((height-50)/size))/2;
