...
import client.ui.element.elements.*;
...
public class Notification extends Module {
	...
	ElementManager gui = client.Client.hud.gui;
	...
	public void addPanel(String str1, String str2) {
		...
		panels.add(new TextPanel(gui, sr.getScaledWidth(), sr.getScaledHeight()-31+4, 106, 30-2, false, mc.fontRendererObj, str1, new Color(ColorUtils.alpha(ThemeManager.getTheme().light(0), 0xff), Mode.EASEOUT)));
		panels.add(new TextPanel(gui, sr.getScaledWidth(), sr.getScaledHeight()-31+4, 106, 30-2, false, mc.fontRendererObj, str1, new Color(ColorUtils.alpha(ThemeManager.getTheme().light(0), 0xff), Mode.EASEOUT)));
		...
		client.Client.hud.gui.addPanel(panels.get(panels.size() - 7));
		client.Client.hud.gui.addPanel(panels.get(panels.size() - 6));
		client.Client.hud.gui.addPanel(panels.get(panels.size() - 5));
		client.Client.hud.gui.addPanel(panels.get(panels.size() - 4));
		client.Client.hud.gui.addPanel(panels.get(panels.size() - 3));
		client.Client.hud.gui.addPanel(panels.get(panels.size() - 2));
		client.Client.hud.gui.addPanel(panels.get(panels.size() - 1));
	}
	...
	public void onEnable() {
		ScaledResolution sr = new ScaledResolution(mc);
		client.Client.hud.gui.panels = new CopyOnWriteArrayList<>();
