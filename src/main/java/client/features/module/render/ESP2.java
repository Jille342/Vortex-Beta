          ...
          ScaledResolution scaledResolution = new ScaledResolution(mc);
            switch (mode.getMode()) {
                case "2D": {
                    GL11.glPushMatrix();
                    collectEntities();
                    float partialTicks = TimerUtils.getTimer().renderPartialTicks;
                                    ...
                                    if (health.enable) {
                                        ...
                                        if (hp > 0.0F) {
                                            ...
                                            if (mc.thePlayer.getDistanceToEntity(entityLivingBase) < 20)
                                                font3.drawStringWithShadow("" + MathHelper.floor_double(hp), posX - 8.0D, endPosY - hpHeight, healthColor);
                                            if (absorption > 0.0F)
...
//                                        mc.getRenderItem().renderItemIntoGUI(stack, endPosX + 4, posY + ydiff - 1 - (diff1 / 2) - 18);
                                            ...
                                            if (stack2 != null) {
                                                ...
                                                if (mc.thePlayer.getDistanceSqToEntity(player) < 20) {
...
//                                        mc.getRenderItem().renderItemIntoGUI(stack4, endPosX + 4, (posY + ydiff * 4) - (diff1 / 2) - 18);
            ...
            switch (currentmode) {
                case "Box":
                    for (Object obj : mc.theWorld.loadedEntityList) {
                        ...
                        if (isValid(entity)) {
                            double x = entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * TimerUtils.getTimer().renderPartialTicks - mc.getRenderManager().viewerPosX;
                            double y = entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * TimerUtils.getTimer().renderPartialTicks - mc.getRenderManager().viewerPosY;
                            double z = entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * TimerUtils.getTimer().renderPartialTicks - mc.getRenderManager().viewerPosZ;
        ...
        if (e instanceof EventNameTag) {
            if ((Boolean) tags.enable && (mode.getMode().equals("2D"))) {
                e.setCancelled(true);
            }
        }
    }
    ...
    private boolean isValid(Entity entity) {
        ...
        if (!(Boolean) invisibles.enable && entity.isInvisible())
            return false;
        if ((Boolean) items.enable && entity instanceof EntityItem && mc.thePlayer.getDistanceToEntity(entity) < 10.0F)
            return true;
